...
public class TestDAO 
{
	...
	public long insert(Test test)
	{
		...
		return DatabaseHelper.get(mAppContext).getWritableDatabase().insert(TABLE_TEST, null, values);
	}
	...
	public int update(Test test)
	{
		...
		String selection = COLUMN_TEST_ID + " LIKE ?";
		String[] selectionArgs = {String.valueOf(test.getId())};
		return DatabaseHelper.get(mAppContext).getReadableDatabase().update(TABLE_TEST, values, selection, selectionArgs);
	}
	
	public Test getTestById(long testId)
	{
		Cursor cursor = DatabaseHelper.get(mAppContext).getReadableDatabase().query(TABLE_TEST, 
				null, // all columns.
				COLUMN_TEST_ID + " = ?", // look for module id
	...
	public Test getTestByName(String testName)
	{
		...
		Cursor cursor = DatabaseHelper.get(mAppContext).getReadableDatabase().query(TABLE_TEST, 
				null, // all columns.
				COLUMN_TEST_NAME + " = ?", // look for module id
